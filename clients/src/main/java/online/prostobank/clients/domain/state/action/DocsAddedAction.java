...
import online.prostobank.clients.domain.events.ApplicationDocumentNotifier;
...
/**
 * Инициируется при переводе в статус "документы готовы", что является сигналом в ПОС для продолжения работы с карточкой
 * силами бэк-менеджера
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class DocsAddedAction implements Action<ClientStates, ClientEvents> {
    ...
    public void execute(StateContext<ClientStates, ClientEvents> context) {
        Optional.of(context)
                ...
                .ifPresent(result -> publisher.publishEvent(new ApplicationDocumentNotifier(result.getId())));
    }
}
